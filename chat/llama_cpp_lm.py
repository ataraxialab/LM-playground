...
class Llama:
    ...
    def _create_completion(
        ...
        prompt_tokens_test = prompt_tokens
        if streamer is not None:
            prompt_tokens_test = np.array(prompt_tokens_test)
            streamer.put(prompt_tokens_test)
        ...
        for token in self.generate(
            ...
            if streamer is not None:
                # streamer.put(torch.tensor([token]), completion_tokens = completion_tokens, prev_tokens=prompt_tokens,detokenize_llama = self)
    ...
    def create_chat_completion_openai_v1(
        ...
        try:
            ...
            if stream:
                return (ChatCompletionChunk(**chunk) for chunk in
